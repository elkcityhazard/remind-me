...
func (sqdb *SQLDBRepo) GetUserRemindersByID(id int64) ([]*models.Reminder, error) {
	...
	go func() {
		...
		stmt := `SELECT ID, Title, Content, UserID, DueDate, CreatedAt, UpdatedAt, Version FROM Reminder WHERE Reminder.UserID = ?`

		rows, err := sqdb.Config.DB.QueryContext(ctx, stmt, id)
